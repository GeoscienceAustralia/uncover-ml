...
@cl.option('--verbose', required=False, help="Log "
            "verbose output")
@cl.argument('outfile', type=cl.Path(exists=False), required=True)
def main(outfile, resolution=None, bbox=None, pointlist=None, 
                   geotiff=None, verbose=False):
    """
    ...
    if geotiff is not None:
        with rasterio.open(geotiff) as raster:
            ...
            log.info("extracted bbox from raster: {} x {}".format(x_range, y_range))
