...
def intersect_and_sample_shp(shp: Path, geotifs: Dict[str, str],
                             dedupe: bool = False,
...
def sum_distance_from_median(x):
    s = np.nansum([v for k, v in x.items() if k.endswith('_distance_from_median')])
    return s

all_pts_with_count_and_median['median_sum'] = all_pts_with_count_and_median.apply(lambda x: sum_distance_from_median(
    x), axis=1)
...
all_pts = all_pts_with_count_and_median.merge(all_pts_with_count_and_median_min_median_sum, how='left', on=['rows',
                                                                                                            'cols'])
...
final_pts = all_pts[all_pts.median_sum==all_pts.median_sum_min]
final_pts_sorted = final_pts.sort_values(['px_count', 'rows', 'cols'], ascending=[False, True, True])
all_pts_sorted = all_pts.sort_values(['px_count', 'rows', 'cols'], ascending=[False, True, True])
