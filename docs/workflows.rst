...
Shapley values can be calculated for any trained supervised model, provided the following a supplied:

- Trained model file - typically produced from the learn workflow
- Shapefile of named points for which Shapley values are to be calculated
- Shapley config YAML file with relevant information for calculation

For more information on Shapley values, please see the relevant section in Interpretable Machine Learning by
Christoph Molnar, online copy `here <https://christophm.github.io/interpretable-ml-book/shapley.html>`_


Config
~~~~~~

Below is an example of what a Shapley config YAML would look like:

.. code:: yaml

    explainer: explainer
    shapefile:
      dir: /path/to/shapefile/
      type: points
      size: 3
    masker:
            type: independent
...
The ``explainer`` block specifies the explanation algorithm to be used for calculation:
    - Currently, the only allowed value is explainer

The ``shapefile`` block specifies the shapefile to be used for calculation:
    - ``dir`` is the path to the shapefile
    - ``type`` is the type of shapefile being passed for calculation
        - Currently, only 'points' is allowed
    - ``size`` is the size n, for a nxn square of pixels to be created for area calculation around the point
        - Can only be used for point shapefiles
        - Can be omitted

The ``masker`` block specifies the method for masking out data as part of calculation:
    - Currently, only independent is supported
...
The ``do_plots`` block specifies whether or not plots need to be produced

Running
~~~~~~~

To run this workflow, a trained model file, a shapley config YAML and a shapefile are required. Call the workflow with:

.. code:: bash

    uncoverml shapley model_file shap_config_yaml

Once the command is complete, outputs will exist in the sub-directory 'shap' for the directoy
specified in the ``output`` block. These outputs will be:

    - Calculated shapley values, stored in a Python dictionary, then pickled and dumped
    - If enabled, plots as .png file, for each point and model output combination
