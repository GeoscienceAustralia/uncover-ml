...
def render_partition(centres, subchunk, n_subchunks, image_out,
...
def run_pipeline(config):
    ...
    outfile_tif = config.name + "_output_clustering"
    ...
    for i in range(n_subchunks):
        ...
        render_partition(centres, i, n_subchunks, image_out, image_settings,
...
def main():
    if len(sys.argv) != 2:
        print("Usage: cpredictionpipeline <configfile>")
        sys.exit(-1)
